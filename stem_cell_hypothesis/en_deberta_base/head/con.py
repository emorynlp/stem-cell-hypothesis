...
def main():
    cdroot()
    # save_dir = 'data/model/mtl/ontonotes_deberta_base_en/all/lw/3'
    # save_dir = 'data/model/mtl/ontonotes_deberta_base_en/con/2'
    # folder = 'static'
    # folder = 'single'
    save_dir = 'data/model/mtl/ontonotes_deberta_base_en/all/lw/2'
    folder = 'joint-con'
...
def calc_acc(save_dir, folder):
    ...
    # save_dir = f'data/model/mtl/ontonotes_deberta_base_en/dep/0'
    ...
    for batch, attentions in cache:
        if records is None:
            ...
            records = ConAcc()
        attn_values = torch.stack(attentions, dim=2)  # B, T, L, H, C
        for b, con in enumerate(batch['constituency']):
            for i, tree in enumerate(con.subtrees(lambda t: t.height() == 2)):
                tree.index = i + 1  # for bos
            for tree in con.subtrees(lambda t: t.height() == 3):
                begin = tree[0].index
                end = tree[-1].index
                t = tree.label()
                h = attn_values[b, begin:end + 1, :, :, :]
                value = h.mean(0)
                if t not in records.label_centroid:
                    records.label_centroid[t] = torch.zeros((num_layers, num_heads, value.size(-1)),
                                                            dtype=torch.float)
                    records.label_correct[t] = torch.zeros((num_layers, num_heads), dtype=torch.float)
    ...
    timer = CountdownTimer(len(cache))
    for batch, attentions in cache:
        attn_values = torch.stack(attentions, dim=2)
        for b, con in enumerate(batch['constituency']):
            for i, tree in enumerate(con.subtrees(lambda t: t.height() == 2)):
                tree.index = i + 1  # for bos
            for tree in con.subtrees(lambda t: t.height() == 3):
