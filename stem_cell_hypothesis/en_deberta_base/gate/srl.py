...
def main():
    ...
    for i in range(3):
        ...
        save_dir = f'data/model/mtl/ontonotes_deberta_base_en/gated/srl/{i + 3}'
        ...
        mtl.load(save_dir, devices=6)
        mtl.config['gradient_accumulation'] = 2
        if 'dep' in mtl.tasks:
        ...
        for k, v in mtl.tasks.items():
            ...
            v.tst = tasks[k].tst
        # mtl.prune_heads()
        metric = mtl.evaluate(save_dir, cache=True, ret_speed=False)[0]
        # metric['heads'] = ScalarMetric(mtl.get_open_gates_rate())
        scores.append(metric)
        gates.append(mtl.get_open_gates_rate())
