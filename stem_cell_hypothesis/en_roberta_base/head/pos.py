...
def main():
    cdroot()
    # save_dir = 'data/model/mtl/ontonotes_roberta_base_en/all/lw/2'
    # save_dir = 'data/model/mtl/ontonotes_roberta_base_en/single/gated/pos/1'
    ...
    draw_acc(overall.cpu().detach().numpy(), 'Weighted Accuracy')
    ...
    for label, freq in records.label_count.most_common():
        ...
        print(f'{acc.max() * 100:.2f}')
        draw_acc(acc.cpu().detach().numpy(), label)
...
def calc_acc(save_dir, folder):
    ...
    for batch, attentions in cache:
        ...
        for b, pos in enumerate(batch['tag']):
            for i, t in enumerate(pos):
                value = attn_values[b, i, :, :, :]
    ...
    records.finalize()
    centroids = torch.stack(tuple(records.label_centroid.values())).to(device)
