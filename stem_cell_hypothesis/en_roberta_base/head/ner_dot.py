...
if not static:
    for i in range(3):
        save_dir = f'data/model/mtl/ontonotes_roberta_base_en/basic/ner/{i}/single/records.pt'
        records = torch.load(save_dir)
        for label, count in records.label_count.items():
            records.label_correct[label] /= count
        rs.append(records)
else:
    save_dir = f'data/model/mtl/ontonotes_roberta_base_en/basic/ner/0/static/records.pt'
...
if static:
    plt.savefig('data/model/mtl/ontonotes_roberta_base_en/basic/ner/0/static/ner-acc-per-layer.pdf')
else:
    plt.savefig('data/model/mtl/ontonotes_roberta_base_en_/ner/0/single/ner-acc-per-layer.pdf')
plt.show()
