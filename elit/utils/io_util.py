...
import elit
from elit.utils.log_util import logger, cprint, remove_color_tag
...
def load_jsonl(path, verbose=False):
    if verbose:
        ...
        src = open(path, encoding='utf-8') if isinstance(path, str) else path
    for i, line in enumerate(src):
        ...
        try:
            ...
            cprint(f'Illegal json format at line {i + 1} : {line}')
            continue
    if not verbose:
        src.close()
...
def download(url, save_path=None, save_dir=elit_home(), prefix=HANLP_URL, append_location=True, verbose=HANLP_VERBOSE):
    if not save_path:
    ...
    if os.path.isfile(save_path):
        ...
        try:
            downloader = Downloader(url, tmp_path, 4, headers={
                'User-agent': f'ELIT/{__version__} ({platform.platform()})'})
            if verbose:
...
def get_resource(path: str, save_dir=elit_home(), extract=True, prefix=HANLP_URL, append_location=True,
    ...
    Args:
      path: A local path (which will returned as is) or a remote URL (which will be downloaded, decompressed then
        returned).
      save_dir: Where to store the resource (Default value = :meth:`elit.utils.io_util.elit_home`)
...
def path_from_url(url, save_dir=elit_home(), prefix=HANLP_URL, append_location=True):
...
class TimingFileIterator(CountdownTimer):
    ...
    def __iter__(self):
        ...
        self.fp = fp = open(self.filepath, encoding='utf-8', errors='ignore')
        line = fp.readline()
        while line:
            yield line
