...
class TorchComponent(Component, ABC):
    ...
    def load_config(self, save_dir, filename='config.json', **kwargs):
        ...
        self.on_config_ready(**merge_dict(self.config, save_dir=save_dir))
    ...
    def fit(self,
        ...
        dev = self.build_dataloader(**merge_dict(config, data=dev_data, batch_size=batch_size, shuffle=False,
                                                 ...
                                                 overwrite=True, split='dev')) if dev_data else None
        if not finetune:
            flash('[yellow]Building model [blink]...[/blink][/yellow]')
            self.model = self.build_model(**merge_dict(config, training=True, trn=trn, overwrite=True))
            flash('')
    ...
    def _savable_config(self):
        ...
        config.update({
            ...
            'hanlp_version': elit.__version__,
        })
        return config
    ...
    def evaluate(self, tst_data, save_dir=None, logger: logging.Logger = None, batch_size=None, output=False,
        ...
        data = self.build_dataloader(**merge_dict(self.config, data=tst_data, batch_size=batch_size, shuffle=False,
                                                  device=self.devices[0], logger=logger, overwrite=True, split='tst'))
        ...
        if cache:
            ...
            memory_cache = []
            for batch in data:
                memory_cache.append(batch)
                timer.log('Loading data into memory [blink][yellow]...[/yellow][/blink]')
            data = memory_cache
        outputs = self.evaluate_dataloader(data, criterion=criterion, filename=filename, output=output, input=tst_data,
                                           ...
                                           **merge_dict(self.config, batch_size=batch_size, metric=metric,
                                                        logger=logger, save_dir=save_dir, **kwargs))
            ...
            if num_samples:
                ...
                if ret_speed:
                    if isinstance(metric, MetricDict):
                        metric['samples/second'] = ScalarMetric(num_samples / elapsed)
                    else:
                        metric = MetricDict({'metric': metric, 'samples/second': ScalarMetric(num_samples / elapsed)})
            else:
                logger.info(f'speed: {len(data) / elapsed:.0f} batches/second')
                if ret_speed:
                    if isinstance(metric, MetricDict):
                        metric['batches/second'] = ScalarMetric(len(data) / elapsed)
                    else:
                        metric = MetricDict({'metric': metric, 'batches/second': ScalarMetric(len(data) / elapsed)})
    ...
    def __call__(self, data, batch_size=None, **kwargs):
        """Predict on data fed by user. This method calls :meth:`~elit.common.torch_component.predict` but decorates
