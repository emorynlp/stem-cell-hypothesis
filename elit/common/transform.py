...
from hanlp_common.constant import EOS, PAD, BOS
...
class VocabDict(SerializableDict):
    ...
    def __init__(self, *args, **kwargs) -> None:
        """A dict holding :class:`elit.common.vocab.Vocab` instances. When used as a transform, it transforms the field
        corresponding to each :class:`elit.common.vocab.Vocab` into indices.

        Args:
            *args: A list of vocab names.
            **kwargs: Names and corresponding :class:`elit.common.vocab.Vocab` instances.
        """
        vocabs = dict(kwargs)
        for each in args:
            vocabs[each] = Vocab()
    ...
    def put(self, **kwargs):
        """Put names and corresponding :class:`elit.common.vocab.Vocab` instances into self.
...
def insert_bos_safely(sample: dict):
    tokens: list = sample['token']
    if tokens[0] != BOS:
        tokens.insert(0, BOS)
    return sample


def append_eos_safely(sample: dict):
    tokens: list = sample['token']
    if tokens[-1] != EOS:
        tokens.append(EOS)
    return sample
