...
from elit.utils.io_util import get_resource, get_exitcode_stdout_stderr, read_tsv_as_sents, run_cmd, pushd
...
def convert_to_stanford_dependency_330(src, dst, language='zh'):
    ...
    convert_to_stanford_dependency(src, dst, language, sp_home)


def convert_to_stanford_dependency(src, dst, language, sp_home):
...
def remove_all_ec(path):
    ...
    with pushd(script):
        run_cmd(f'java -cp elit-ddr-0.0.5-SNAPSHOT.jar:elit-sdk-0.0.5-SNAPSHOT.jar:elit-1.7.8.jar:'
