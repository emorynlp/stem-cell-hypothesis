...
from elit.layers.transformers.pt_imports import AutoTokenizer, PreTrainedTokenizer, AutoModel_
import torch
...
from elit.common.transform import LowerCase, FieldLength, PunctuationMask
from elit.common.vocab import Vocab
from elit.components.parsers.alg import decode_dep
from elit.components.parsers.biaffine.biaffine_model import BiaffineDependencyModel
from hanlp_common.conll import CoNLLWord, CoNLLSentence
from elit.datasets.parsing.conll_dataset import CoNLLParsingDataset, append_bos
...
class BiaffineDependencyParser(TorchComponent):
    ...
    def build_vocabs(self, dataset, logger=None, transformer=None):
        ...
        if token_vocab and self.config.get('min_freq', None):
            ...
            freq_words = reserved_token + [token for token, freq in counter.items() if freq >= self.config.min_freq]
