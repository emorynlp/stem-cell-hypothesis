...
from elit.common.dataset import PadSequenceDataLoader, PrefetchDataLoader, CachedDataLoader, MultiTaskDataLoader
from hanlp_common.document import Document
from elit.common.structure import History
from elit.common.torch_component import TorchComponent
from elit.common.transform import FieldLength, TransformList, insert_bos_safely, append_eos_safely
...
from hanlp_common.util import merge_locals_kwargs, topological_sort, reorder, prefix_match, merge_dict
...
class MultiTaskLearning(TorchComponent):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        into this MTL framework, a component needs to implement the :class:`~elit.components.mtl.tasks.Task` interface.
    ...
    def build_dataloader(self,
        ...
        for i, (task_name, task) in enumerate(self.tasks.items()):
            ...
            task_dataloader = self.build_task_dataloader(task_name, task, data, device, gradient_accumulation, logger,
                                                         cache, debug, training)
        ...
        if data == 'trn':
            ...
            logger.info('\n'.join(rows))
        if prefetch and ((data == 'trn') or (data == 'dev' and not self.config.get('tasks_need_custom_eval', None))):
            dataloader = PrefetchDataLoader(dataloader, prefetch=prefetch)

        return dataloader

    def build_task_dataloader(self,
                              task_name,
                              task,
                              data,
                              device,
                              ...
                              debug=False,
                              training=False):
        encoder_transform, transform = self.build_transform(task, training)
        training = None
        if data == 'trn':
            if debug:
                _data = task.dev
    ...
    def build_transform(self, task: Task, training=False) -> Tuple[TransformerSequenceTokenizer, TransformList]:
        ...
        if training:
            if self.config.get('self_teaching', None):
                encoder_transform.cls_is_bos = any([x.cls_is_bos for x in self.tasks.values()])
                if encoder_transform.cls_is_bos and not task.cls_is_bos:
                    transform.insert(0, insert_bos_safely)
                encoder_transform.sep_is_eos = any([x.sep_is_eos for x in self.tasks.values()])
                if encoder_transform.sep_is_eos and not task.sep_is_eos:
                    transform.insert(0, append_eos_safely)
    ...
    def _collect_encoder_parameters(self):
        return list(x for x in self.model_.encoder.parameters() if x.requires_grad)
    ...
    def build_optimizer(self,
                        ...
                        lr,
                        encoder_lr,
                        self_teaching: SelfTeaching = None,
                        **kwargs):
        model = self.model_
        num_training_steps = len(trn) * epochs // self.config.get('gradient_accumulation', 1)
        encoder_parameters = self._collect_encoder_parameters()
        ...
        if self_teaching:
            self_teaching.on_trn_begin(trn, device=self.device)
            if self_teaching.loss_weighted_sum:
                # Either put loss sum into encoder optimizer or decoder, let's try encoder
                parameter_groups.append({"params": self_teaching.loss_weighted_sum.parameters(),
                                         'lr': encoder_lr, 'weight_decay': 0.0})

        # noinspection PyTypeChecker
        encoder_optimizer = optimization.AdamW(
            self._finalize_encoder_parameter_groups(parameter_groups),
            lr=lr,
    ...
    def execute_training_loop(self,
        ...
        best_epoch, best_metric = 1, -1
        timer = CountdownTimer(epochs)
        ratio_width = len(f'{len(trn)}/{len(trn)}')
        epoch = 0
        history = self.build_history(save_dir)
        for epoch in range(1, epochs + 1):
            logger.info(f"[yellow]Epoch {epoch} / {epochs}:[/yellow]")
            self.fit_dataloader(trn, criterion, optimizer, metric, logger, history, ratio_width=ratio_width,
                                **merge_dict(self.config, overwrite=True, self_teaching=self_teaching))
            if dev:
                # if self_teaching:  # Universal cls/sep is used only in training, so let's restore it for dev
                #     _backup = dict()
                #     for task_name, task in self.tasks.items():
                #         _backup[task_name] = (task.cls_is_bos, task.sep_is_eos)
                #         task.cls_is_bos = task.config.cls_is_bos
                #         task.sep_is_eos = task.config.sep_is_eos
                self.evaluate_dataloader(dev, criterion, metric, logger, ratio_width=ratio_width, input='dev',
                                         history=history, save_dir=save_dir)
                if self_teaching:  # Then restore these for training
                    # for task_name, task in self.tasks.items():
                    #     # noinspection PyUnboundLocalVariable
                    #     task.cls_is_bos, task.sep_is_eos = _backup[task_name]
                    self_teaching.on_trn_epoch_end(epoch, metric, self, logger)
            report = f'{timer.elapsed_human}/{timer.total_time_human}'
            dev_score = metric.score
            if save_after_each_epoch:
                _digits = len(str(epochs))
                self.save_weights(save_dir, filename='model-' + '0' * (_digits - len(str(epoch))) + str(epoch) + '.pt')
        ...
        if best_epoch != epoch:
            ...
            if save_after_each_epoch:
                ...
                self.load_weights(save_dir,
                                  filename='model-' + '0' * (_digits - len(str(best_epoch))) + str(best_epoch) + '.pt')
            else:
                self.load_weights(save_dir)
        return best_metric
    ...
    def fit_dataloader(self,
                       ...
                       patience=0.5,
                       eval_trn=False,
                       loss_balancer: MovingAverageBalancer = None,
                       self_teaching: SelfTeaching = None,
                       temperature_function=None,
                       kd_loss_function=None,
                       **kwargs):
        self.model.train()
        ...
        if self_teaching:
            cls_is_bos = any([x.cls_is_bos for x in self.tasks.values()])
            sep_is_eos = any([x.sep_is_eos for x in self.tasks.values()])
        for idx, (task_name, batch) in enumerate(trn):
            decoder_optimizer = decoder_optimizers.get(task_name, None)
            if self_teaching:
                raw_batch = batch
                # noinspection PyUnboundLocalVariable
                output_dict, batch = self.feed_batch(batch, task_name, cls_is_bos=cls_is_bos, sep_is_eos=sep_is_eos,
                                                     run_transform=True)
            else:
            ...
            loss = self.compute_loss(batch, output_dict[task_name]['output'], criterion[task_name],
                                     self.tasks[task_name], task_name, history=history)
            if self_teaching and self_teaching.passed_after_epochs:
                # noinspection PyUnboundLocalVariable
                loss = self_teaching.compute_kd_loss(loss, task_name, raw_batch, cls_is_bos, sep_is_eos, output_dict,
                                                     temperature_function, kd_loss_function, self)
            if loss_balancer:
    ...
    def build_model(self, training=False, model_cls=MultiTaskModel, **kwargs) -> MultiTaskModel:
        ...
        encoder.ret_raw_hidden_states = encoder.ret_raw_hidden_states or any(use_raw_hidden_states.values())
        return model_cls(encoder.module(training=training), scalar_mixes, decoders, use_raw_hidden_states)
        ...
        if isinstance(dependencies, str):
            if dependencies in self.tasks:
                ...
                assert prefix_matched, f'No prefix matching for {dependencies}. ' \
                                       f'Check your dependencies definition: {list(self.tasks.keys())}'
    ...
    def fit(self,
            ...
            tasks_need_custom_eval=None,
            loss_balancer=None,
            self_teaching: Union[int, bool, SelfTeaching] = False,
            temperature_function=None,
            kd_loss_function=None,
        ...
        if self_teaching is True:
            self_teaching = max(int(0.5 * epochs), epochs - 10)  # use the last 10 epochs to teach itself
        if self_teaching:
            kd_loss_function = kd_loss_function or 'kd_ce_loss'
            temperature_function = temperature_function or 'flsw'
        if self_teaching and isinstance(self_teaching, int):
            self_teaching = SelfTeaching(self_teaching, cache_prefix=cache)
    ...
    def compute_loss(self,
                     ...
                     task: Task,
                     task_name: str,
                     history: History = None) -> torch.FloatTensor:
